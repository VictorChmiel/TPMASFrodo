...
package frodo2.algorithms.asodpop.goodsTree.innerNodeTreeBinaryDomains;
...
import frodo2.algorithms.odpop.goodsTree.InnerNodeTreeFullDomain.InnerNode;
...
extends frodo2.algorithms.odpop.goodsTree.InnerNodeTreeBinaryDomains.InnerNodeTree<Val, U, LeafNode<U>> {
	 ...
	 */
	public int add(Good<Val, U> g, int sender, HashMap<String, Val[]> domains) {
		...
		if(numberOfVariables > 0) {
			...
			for(Val[] aValues : reportedValues) {
				...
				for(String var : aVariables) {
					...
					if(!variableToDepth.containsKey(var)) {
						...
						if(!childrenVariables[sender][varIndex]) {
							updateInfo = true;
							additionalVariables.add(var);
							ArrayList<Val> domain = this.domains.get(var);
				...
				if(newVariable) {
					...
					indexPath = addNewVariable(aVariables, newVariables, newDomains, sender);
				}
				...
				if(updateInfo) {
					...
					int size = additionalVariables.size();
				...
				assert !root.isAlive() || !root.hasUB() || oldUB == null || greaterThanOrEqual(root.getUB(), oldUB);
				...
				if(this.storeReceivedGoods)
					this.storeReceivedGoods = this.upperBoundIsInfiniteCounter != 0;
				...
				assert !root.hasUtil() || !root.hasUB() || !root.getMaxUB().isUpToDate() || greaterThanOrEqual(root.getMaxUB().getUtil(), root.getUtil());
			}
		}
	...
	public Good<Val, U> getAmax() {
		...
		if(this.upperBoundIsMinInfiniteCounter == 0 && this.hasLocalProblem && (!root.hasUtil() || (this.numberOfChildren > 0 && ((maximize && greaterThan(root.getUtil(), this.optimalLocalUtility)) || (!maximize && this.localUpperBound != null && greaterThan(root.getUtil(), this.localUpperBound)))  ))) {
...
//					assert !ownVariables[0] || localRemoved || this.checkGood(optimalLocalSolution); 
	 ...
	 */
	@Override
	protected LeafNode<U> createLeaf(IntArrayWrapper currentPath, frodo2.algorithms.odpop.Good<Val, U> g, int child, final boolean withUB) {
	 ...
	 */
	@Override
	protected LeafNode<U> createLeaf(IntArrayWrapper currentPath, final boolean withUB) {
	 ...
	 * @param optimalPath the path to the optimal solution
	...
	/**
	 * @see frodo2.algorithms.odpop.goodsTree.InnerNodeTreeBinaryDomains.InnerNodeTree#updateLocalProblem()
	 */
	@Override
	...
	public boolean checkLeaf(LeafNode<U> leaf, IntArrayWrapper currentPath,  boolean checkUB, U utility, int sender) {
		...
		assert !checkUB || greaterThan(UB, util) || leaf.getUtil().equals(util) : UB.toString() + " vs. " + util + " OR " + leaf.getUtil() + " != " + util;	// the utility should be correct
