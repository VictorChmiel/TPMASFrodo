...
public class ASODPOPTest < V extends Addable<V>, U extends Addable<U> > extends TestCase implements IncomingMsgPolicyInterface<String> {
	 ...
	 */
	public ASODPOPTest(String method, Class<V> domClass, Class<U> utilClass) {
		super (method);
	...
	static public TestSuite suite () {
		TestSuite testSuite = new TestSuite ("Tests for ASODPOP");
		...
		testTmp.addTest(new RepeatedTest (new ASODPOPTest<AddableInteger, AddableInteger> ("testRandomSharedMemory", AddableInteger.class, AddableInteger.class), 100));
		...
		testTmp.addTest(new RepeatedTest (new ASODPOPTest<AddableReal, AddableInteger> ("testRandomSharedMemory", AddableReal.class, AddableInteger.class), 100));
		testSuite.addTest(testTmp);
		
		testTmp = new TestSuite ("Tests for the ASODPOP propagation protocol using shared memory pipes and real utilities");
		testTmp.addTest(new RepeatedTest (new ASODPOPTest<AddableInteger, AddableReal> ("testRandomSharedMemory", AddableInteger.class, AddableReal.class), 100));
		testSuite.addTest(testTmp);
		
		testTmp = new TestSuite ("Tests for the ASODPOP propagation protocol using TCP pipes and integer utilities");
		testTmp.addTest(new RepeatedTest (new ASODPOPTest<AddableInteger, AddableInteger> ("testRandomTCP", AddableInteger.class, AddableInteger.class), 50));
		testSuite.addTest(testTmp);
		...
		testTmp.addTest(new RepeatedTest (new ASODPOPTest<AddableInteger, AddableInteger> ("testRandomSharedMemoryNoXML", AddableInteger.class, AddableInteger.class), 100));
		testSuite.addTest(testTmp);
	...
	public void testRandomSharedMemory () throws Exception {
		test (ASODPOP.DetermineAssignmentMax.class.getName(), false, true);
	}
	...
	public void testRandomTCP () throws Exception {
		test (ASODPOP.DetermineAssignmentMax.class.getName(), true, true);
	}
	...
	public void testRandomSharedMemoryNoXML () throws Exception {
		test (ASODPOP.DetermineAssignmentMax.class.getName(), false, false);
	}
	...
	private void test (String combination, boolean useTCP, boolean useXML) throws Exception {
		...
		for (String agent : parser.getAgents()) {
			...
			if (useXML) { // use the XML-based constructor
				...
				Constructor<?> constructor = ASODPOP.class.getConstructor(parTypes);
				Object[] args = new Object[2];
				args[0] = subprob;
				args[1] = parameters;
				queue.addIncomingMessagePolicy((ASODPOP<V, U>) constructor.newInstance(args));
				...
				queue.addIncomingMessagePolicy(new ASODPOP<V, U> (subprob));
			}
		}
