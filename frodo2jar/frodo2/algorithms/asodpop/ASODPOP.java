 ...
 *
 */
public class ASODPOP < Val extends Addable<Val>, U extends Addable<U> >
implements StatsReporterWithConvergence<Val> {
	...
	public static final String UTIL_MSG = UTILpropagationFullDomain.UTIL_MSG;
	
	/** The type of the UTIL message with domain information */
	public static final String UTIL_MSG_DOM = UTILpropagationFullDomain.UTIL_MSG_DOM;
	
	/** The type of the UTIL message with variable IDs*/
	public static final String UTIL_MSG_VARS = UTILpropagationFullDomain.UTIL_MSG_VARS;
	
	/** The type of the UTIL message with variable IDs and domain information */
	public static final String UTIL_MSG_DOM_VARS = UTILpropagationFullDomain.UTIL_MSG_DOM_VARS;

	/** The type of the ASK message*/
	public static final String ASK_MSG = UTILpropagationFullDomain.ASK_MSG;

	/** The type of the DONE message*/
	public static final String DONE_MSG	= UTILpropagationFullDomain.DONE_MSG;
	...
	public static final String CONV_STATS_MSG_TYPE = "ASODPOPConvStatsMsg";
	 ...
	 */
	public ASODPOP (Element parameters, DCOPProblemInterface<Val, U> problem) {
	 ...
	 */
	public ASODPOP (DCOPProblemInterface<Val, U> problem) throws Exception  {
		this.problem = problem;
		setCombinationMethod(ASODPOP.DetermineAssignmentMax.class.getName());
		this.convergence = false;
	}
	 ...
	 * @throws Exception 	if an error occurs
	 */
	public ASODPOP (DCOPProblemInterface<Val, U> problem, String combinationMethodClass) throws Exception  {
		this.problem = problem;
	...
	private void setCombinationMethod(String method) throws ClassNotFoundException, NoSuchMethodException, IllegalArgumentException, InstantiationException, IllegalAccessException, InvocationTargetException {
		if(method == null)
			method = ASODPOP.DetermineAssignmentMax.class.getName();
	...
	public void notifyIn (Message msg) {
		...
		if (type.equals(ASODPOP.OUTPUT_MSG_TYPE)) { // in stats gatherer mode, the message containing information about an agent's assignments
		...
		else if (type.equals(ASODPOP.CONV_STATS_MSG_TYPE)) { // in stats gatherer mode, the message sent by a variable containing the assignment history
		...
		if(type.equals(UTIL_MSG)) {	// Receiving a new good
		...
		else if(type.equals(UTIL_MSG_DOM)) {	// Receiving a new good
			...
			if(ready[varIndex]) {
				...
				String sender = msgCast.getSender();
				String[] variables = ((InnerNodeTree<Val, U>)variable.tree).getChildSeparatorReportingOrder(variable.childrenPointer.get(sender));
				processUTILMessage(variable, msgCast.getGood(variables), msgCast.getSender(), domInfo, variables, false);
			} else
				goodsToProcess[varIndex].add(msgCast);
		} 
			...
			if(ready[varIndex]) {
				...
				sendMessageToVariable(child, variable.getAckMessage(childPointer)); /// @todo Reusing messages has become useless
				((InnerNodeTree<Val, U>)variable.tree).setChildrenSeparator(childPointer, msgCast.getVariables());
				processUTILMessage(variable, msgCast.getGood(), msgCast.getSender(), null, null, true);
			} else {
		...
		else if(type.equals(UTIL_MSG_DOM_VARS)) {	// Receiving a new good
			...
			if(ready[varIndex]) {
				...
				int childPointer = variable.childrenPointer.get(child);
				sendMessageToVariable(child, variable.getAckMessage(childPointer)); /// @todo Reusing messages has become useless
				((InnerNodeTree<Val, U>)variable.tree).setChildrenSeparator(variable.childrenPointer.get(msgCast.getSender()), variables);
				processUTILMessage(variable, msgCast.getGood(), msgCast.getSender(), domInfo, variables, true);
			} else {
				sendMessageToVariable(child, new ACKmsg(child));
				goodsToProcess[varIndex].add(msgCast);
			}
...
//			this.dfsReceived++;
	...
	private void startProcess(String varID) {
		...
		if(var.parent == null && var.children.length == 0) { // This variable is a singleton variable.
			...
			while(goodsReceived.size() > 0) {
				...
				((InnerNodeTree<Val, U>)variable.tree).add(good, senderIndex);
			}
	 ...
	 */
	private void processUTILMessage(VariableInfo variable, Good<Val, U> g, String sender, int[] domInfo, String[] variables, boolean withVars) {
		if(!variable.terminated) {
			...
			int nbrNewVariables = ((InnerNodeTree<Val, U>)variable.tree).add(g, childPointer);
...
//				combinationMethod.determineVariableValue(varIndex, variableDomainSize[varIndex]);
		...
		if(!variable.terminated) {
			...
			if(variable.root) {
				...
				if(variable.tree.isValuationSufficient() || variable.infeasible) {
					...
					if(convergence) {
						assignmentHistory[varIndex].add(new CurrentAssignment<Val>(queue.getCurrentTime(), currentValue));
						queue.sendMessage(AgentInterface.STATS_MONITOR, new StatsReporterWithConvergence.ConvStatMessage<Val>(ASODPOP.CONV_STATS_MSG_TYPE, variableID, assignmentHistory[variablePointer.get(variableID)]));
					}
		...
		if(variable.lastSent == null || !g.equals(variable.lastSent)) {
			...
			if(variable.getNewVariable()) {
				...
				if(variable.tree.stillToSend() && variable.tree.hasFullInfo()) {
					newMsg = new UTILvarsMsgWithDom<Val, U>(variable.variableID, variable.parent, g, variable.tree.getFinalDomainSize());
				} else
					newMsg = new UTILvarsMsg<Val, U>(variable.variableID, variable.parent, g);
			} else {
				if(variable.tree.stillToSend() && variable.tree.hasFullInfo()) {
					newMsg = new UTILdomMsg<Val, U>(variable.variableID, variable.parent, g, variable.tree.getFinalDomainSize());
				} else
	...
	private void sendASK(String child, VariableInfo var) {
		...
		sendMessageToVariable(child, var.getAskMessage(child)); /// @todo Reusing messages has become useless
	}
	...
	implements determineAssignment<Val, U> {
		 ...
		 */
		private ASODPOP<Val, U> asodpop;

		/**
		 * A constructor
		 * @param asodpop		Link to the ASODPOP class
		 */
		public DetermineAssignmentMax(ASODPOP<Val, U> asodpop) {
		 ...
		 * 
		 * @see frodo2.algorithms.asodpop.ASODPOP.determineAssignment#determineVariableValue(int, int)
		 */
		public Val determineVariableValue(int varIndex, int domainSize) {
			ASODPOP<Val, U>.VariableInfo variable = asodpop.infos[varIndex];
		...
		/**
		 * @see frodo2.algorithms.asodpop.ASODPOP.determineAssignment#setZero(frodo2.solutionSpaces.Addable)
		 */
		public void setZero(U zero) {} 
	...
	/**
	 * Assignment message used to reported the variables assignment
	 * and some statistics about the run
	 * @author Brammert Ottens, 26 feb 2010
	 * 
	 * @param <Val> type of domain values
	 * @param <U>   type of utility values
	 */
	public static class AssignmentMessage < Val extends Addable<Val>, U extends Addable<U> >
	extends VALUEpropagation.AssignmentMessage<Val, U> {
		...
		/** @see frodo2.algorithms.odpop.VALUEpropagation.AssignmentMessage#writeExternal(java.io.ObjectOutput) */
		@Override
		public void writeExternal(ObjectOutput out) throws IOException {
			super.writeExternal(out);
			out.writeInt(this.numberOfSpeculativeUTIlmessages);
			out.writeInt(this.numberOfUTILmessages);
		}

		/** @see frodo2.algorithms.odpop.VALUEpropagation.AssignmentMessage#readExternal(java.io.ObjectInput) */
		@Override
		public void readExternal(ObjectInput in) throws IOException, ClassNotFoundException {
			super.readExternal(in);
			this.numberOfSpeculativeUTIlmessages = in.readInt();
			this.numberOfUTILmessages = in.readInt();
		}
		 ...
		 * @param numberOfSpeculativeUTIlmessages number of speculative UTIl messagegs received
		 * @param numberOfUTILmessages total number of UTIL messages
		 */
		public AssignmentMessage (String var, Val val, double treeFillPercentage, double dummyFillPercentage, long numberOfDummies, int numberOfSpeculativeUTIlmessages, int numberOfUTILmessages) {
			super(var, val, treeFillPercentage, dummyFillPercentage, numberOfDummies);
			this.numberOfSpeculativeUTIlmessages = numberOfSpeculativeUTIlmessages;
			this.numberOfUTILmessages = numberOfUTILmessages;
		}
		 ...
		 * @return the total number of speculative UTIL messages that have been sent
		 */
		public int getNumberOfSpeculativeUTIlmessages() {
			return numberOfSpeculativeUTIlmessages;
		}
		
		/** @return total number of UTIL Messages */
		public int getNumberOfUTILmessages() {
			return this.numberOfUTILmessages;
		}
	}
