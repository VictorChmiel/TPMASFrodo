...
public class VALUEpropagationTest < V extends Addable<V>, U extends Addable<U> > extends TestCase implements IncomingMsgPolicyInterface<String> {
	 ...
	 */
	public VALUEpropagationTest(boolean useTCP, boolean useXML, Class<V> domClass, Class<U> utilClass) {
		super ("test");
		this.useTCP = useTCP;
		this.useXML = useXML;
	...
	protected void setUp() throws Exception {
		...
		/// @todo Also test when the signs of utilities are unrestricted, but this is currently not supported
		problem = AllTests.generateProblem(graph, true, +1);
	...
	static public TestSuite suite () {
		...
		TestSuite testTmp = new TestSuite ("Tests for the VALUEpropagation protocol using shared memory pipes and integer utilities");
		testTmp.addTest(new RepeatedTest (new VALUEpropagationTest<AddableInteger, AddableInteger> (false, true, AddableInteger.class, AddableInteger.class), 1000));
		testSuite.addTest(testTmp);
		
		testTmp = new TestSuite ("Tests for the VALUEpropagation protocol using shared memory pipes and integer utilities and real-value variables");
		testTmp.addTest(new RepeatedTest (new VALUEpropagationTest<AddableReal, AddableInteger> (false, true, AddableReal.class, AddableInteger.class), 1000));
		testSuite.addTest(testTmp);
		
		testTmp = new TestSuite ("Tests for the VALUEpropagation protocol using TCP pipes and integer utilities");
		testTmp.addTest(new RepeatedTest (new VALUEpropagationTest<AddableInteger, AddableInteger> (true, true, AddableInteger.class, AddableInteger.class), 50));
		testSuite.addTest(testTmp);
		
		testTmp = new TestSuite ("Tests for the VALUEpropagation protocol using shared memory pipes and integer utilities and the alternative constructor");
		testTmp.addTest(new RepeatedTest (new VALUEpropagationTest<AddableInteger, AddableInteger> (false, false, AddableInteger.class, AddableInteger.class), 1000));
		testSuite.addTest(testTmp);
		
		testTmp = new TestSuite ("Tests for the VALUEpropagation protocol using shared memory pipes and real utilities");
		testTmp.addTest(new RepeatedTest (new VALUEpropagationTest<AddableInteger, AddableReal> (false, true, AddableInteger.class, AddableReal.class), 1000));
		testSuite.addTest(testTmp);
	...
	throws IOException, NoSuchMethodException, IllegalArgumentException, InstantiationException, IllegalAccessException, ClassNotFoundException, InvocationTargetException {
		...
		for (String agent : parser.getAgents()) {
			...
			if (useXML) { // use the XML-based constructor
				...
				parTypes[0] = DCOPProblemInterface.class;
				parTypes[1] = Element.class;
				constructor = VALUEpropagation.class.getConstructor(parTypes);
				args = new Object[2];
				args[0] = subprob;
				args[1] = parameters;
				queue.addIncomingMessagePolicy((VALUEpropagation<V, U>) constructor.newInstance(args));
				...
				queue.addIncomingMessagePolicy(new UTILpropagation<V, U> (subprob, parameters));
				queue.addIncomingMessagePolicy(new VALUEpropagation<V, U> (subprob, parameters));
		...
		// check that the assignment that has been found is correct
