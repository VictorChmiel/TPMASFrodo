 ...
 */
public class SamplingChild <V extends Addable<V>> extends SamplingPruning<V> {
	 ...
	 */
	public SamplingChild(Element parameters, DCOPProblemInterface<V, AddableReal> problem) {
	 ...
	 * @param parameters	listeners parameters (not used for the moment)
	 */
	public SamplingChild(DCOPProblemInterface<V, AddableReal> problem, Element parameters) {
		...
		public boolean storeCOSTmsg(COSTBmsg msg) {
			...
			if(++this.costMessagesReceived == nbrChildren) {
				...
				if(!node.random) {
					...
					if(node.feasible)
						nextCostMsg = new COSTBmsg(parent, cost == null ? penalty : cost, new AddableReal(-1));
					else
						nextCostMsg = new COSTBmsg(parent, null, new AddableReal(node.maxBound));
				}
				return true;
