 ...
 */
public class BoundInvLog <V extends Addable<V>> implements Bound<V> {
	 ...
	 * Constructor
	 * 
	 * @param size Estimate for lambda(A)
	 */
	public BoundInvLog(Long size){
	...
	public double sampleBound(int index, SearchNode<V> node) {
		double gamma = Math.log(node.actionFrequencies[index] + 1);
		assert 1/gamma - Math.pow(node.frequencie/(size*gamma*gamma), 2) >= 0 : size + " < " + 0;
		return 1 + 1/gamma - Math.pow(node.frequencie/(size*gamma*gamma), 2);
	}
