 ...
 */
public class BoundSqrtSize <V extends Addable<V>> implements Bound<V> {
	 ...
	 */
	public BoundSqrtSize(Long size) {
		this.size = size;
	}
	...
	public double sampleBound(int index, SearchNode<V> node) {
		return Math.sqrt(size*Math.sqrt(2*node.frequencie)/node.actionFrequencies[index]);
	}
