...
import frodo2.algorithms.duct.NormalizeInf;
 ...
 * 
 */
public class NormalizeInfTest extends TestCase implements IncomingMsgPolicyInterface<String> {
	 ...
	 */
	public NormalizeInfTest(boolean useTCP) {
		super("test");
		this.useTCP = useTCP;
	}
	...
	static public TestSuite suite () {
		...
		testTmp.addTest(new RepeatedTest (new NormalizeInfTest (false), 500));
		testSuite.addTest(testTmp);
		
		
		testTmp = new TestSuite ("Tests for the Normalization protocol using TCP pipes");
		testTmp.addTest(new RepeatedTest (new NormalizeInfTest (true), 500));
		testSuite.addTest(testTmp);
	...
	throws IOException, NoSuchMethodException, IllegalArgumentException, InstantiationException, IllegalAccessException, ClassNotFoundException, InvocationTargetException {
		...
		for (String agent : parser.getAgents()) {
			...
			Constructor<?> constructor = NormalizeInf.class.getConstructor(parTypes);
			Object[] args = new Object[3];
			args[0] = subprob;
			args[1] = parameters;
			args[2] = true;
			queue.addIncomingMessagePolicy((NormalizeInf<AddableInteger>) constructor.newInstance(args));
		}
	...
	public Collection<String> getMsgTypes() {
		...
		msgTypes.add(NormalizeInf.OUT_MSG_TYPE);
		return msgTypes;
	}
	...
	public void notifyIn(Message msg) {
			...
			if(type.equals(NormalizeInf.OUT_MSG_TYPE)) {
