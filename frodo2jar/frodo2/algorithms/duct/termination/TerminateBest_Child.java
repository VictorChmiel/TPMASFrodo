 ...
 * 
 */
public class TerminateBest_Child <V extends Addable<V>> implements TerminationCondition<V> {
	...
	if(dist.convergenceBound(dist.maxValueIndex, delta) > delta && ((SearchNodeChild<V>)dist).childB[dist.maxValueIndex] != dist.maxValue)
		return false;
	...
	for(int i = 0; i < dist.numberOfValues; i++) {
		...
		if(maximize) {
			...
			bound = bound < childBound ? childBound : bound;
		}
